...
\vspace{20mm}
<<fig = TRUE , height = 3, results=hide>>=
setwd("C:/Users/BrewJR/Documents/fetalmed/protocol")
library(rgdal)
bcn <- readOGR("spatial", "BCN_Barri_SHP")

comarca <- readOGR("spatial/muni", "bm50mv33sh1fpc1r170")
muni <-readOGR("spatial/muni", "bm50mv33sh1fpm1r170")

my_colors <- colorRampPalette(c("darkred", "white", "grey"))(nrow(bcn))
my_colors <- colorRampPalette(c("brown", "grey", "lightblue"))(nrow(muni))
my_colors <- sample(my_colors, length(my_colors))


# Also read in spanish map
library(devtools)
# For creating dot map
source_url("https://raw.githubusercontent.com/joebrew/maps/master/gadm_function.R")
gadm(country = "SPAIN", level = 1)

# Convert catalunya's map projection to Spains
comarca <- spTransform(comarca, CRS(proj4string(spain1)))
muni <- spTransform(muni, CRS(proj4string(spain1)))

par(mar = c(0,0,0,0))
plot(muni, col = my_colors, border = NA)
plot(comarca, add = T, lwd = 0.2)
...
% 
% boundary <- collapse_map(bcn)
% 
% # Define color vector
% my_colors <- colorRampPalette(c("grey", "darkred"))(100)
% 
% SurfaceFun <- function(var = bcn$Dones * coordinates(bcn)[,1] ,
...
%   boundary_points <- boundary_points[[2]]@coords
%   
%   }else{
%   boundary_shape <- spain1[which(spain1$NAME_1 == "Cataluña"),]
% 
%   boundary_points <- boundary_shape@polygons[[1]]@Polygons
%   boundary_points <- boundary_points[[10]]@coords
%   
%   }
%   # Getting the site counts 
%   shape$lon <- coordinates(shape)[,1]
%   shape$lat <- coordinates(shape)[,2]
% 
%   # Get trap locations and data values
%   a <- data.frame("x" = shape$lon,
%                   "y" = shape$lat,
%                   "z" = var)
...
%          legend = rep(NA, 10), #c(legtemp[1], NA, NA, legtemp[4], NA, NA, legtemp[7], NA, NA, legtemp[10]),
%          border = FALSE,
%          bty = "n",
%          ncol = 1,
%          y.intersp = 0.5,
...
% par(mfrow = c(1,2))
% # plot(bcn,
% #      xlim = c(420000, 436000),
% #      ylim = c(4575000, 4591000))
% DotFun(main_shape = bcn,
%        points_number = 1000,
%        points_var = (bcn$Dones) ^ 0.8,
%        border = "grey")
% 
% 
% SurfaceFun()
% box("outer")
% title(main = "Barcelona", outer = T)
% @
% \end{center}
% 
% 
...
% <<fig = TRUE, height = 3, results = hide>>=
% my_colors <- colorRampPalette(c("brown", "grey", "lightblue"))(nrow(muni))
% boundary <- collapse_map(comarca)
% 
% par(oma = c(0,0,2,0))
% par(mar = c(0,0,0,0))
% par(mfrow = c(1,2))
% 
% DotFun(main_shape = comarca,
%        points_number = 1000,
%        points_var = jitter((comarca$SUP_COMAR/10) ^ 2, factor = 5),
%        border = "grey")
% SurfaceFun(var = comarca$SUP_COMAR ,
%                        boundary_shape = boundary,
%            shape = comarca,
%            barcelona = FALSE,
%            legend = FALSE)
% box("outer")
% title(main = "Catalonia", outer = T)
% 
% @
% \end{center}
\includegraphics[width = 180]{maps.jpg}
